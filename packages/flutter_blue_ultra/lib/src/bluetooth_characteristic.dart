...
class BluetoothCharacteristic {
  ...
  List<int> get lastValue {
    ...
    return FlutterBlueUltra._lastChrs[remoteId]?[key] ?? [];
  }
  ...
  ///   - anytime `read()` is called
  ///   - anytime a notification arrives (if subscribed)
  Stream<List<int>> get onValueReceived => FlutterBlueUltraPlatform.instance.onCharacteristicReceived
      .where((p) => p.remoteId == remoteId)
      .where((p) => p.primaryServiceUuid == primaryServiceUuid)
      .where((p) => p.serviceUuid == serviceUuid)
      .where((p) => p.characteristicUuid == characteristicUuid)
  ...
  Future<List<int>> read({int timeout = 15}) async {
    ...
    try {
      ...
      var responseStream = FlutterBlueUltraPlatform.instance.onCharacteristicReceived
      ...
      // invoke
      await FlutterBlueUltra._invokePlatform(() => FlutterBlueUltraPlatform.instance.readCharacteristic(request));
      ...
      // failed?
      if (!response.success) {
        throw FlutterBlueUltraException(_nativeError, "readCharacteristic", response.errorCode, response.errorString);
      }
  ...
  Future<void> write(List<int> value,
    ...
    try {
      ...
      var responseStream = FlutterBlueUltraPlatform.instance.onCharacteristicWritten
          .where((p) => p.remoteId == request.remoteId)
          .where((p) => p.primaryServiceUuid == request.primaryServiceUuid)
          .where((p) => p.serviceUuid == request.serviceUuid)
          .where((p) => p.characteristicUuid == request.characteristicUuid)
      ...
      // invoke
      await FlutterBlueUltra._invokePlatform(() => FlutterBlueUltraPlatform.instance.writeCharacteristic(request));
      ...
      // failed?
      if (!response.success) {
        throw FlutterBlueUltraException(_nativeError, "writeCharacteristic", response.errorCode, response.errorString);
      }
  ...
  Future<bool> setNotifyValue(bool notify, {int timeout = 15, bool forceIndications = false}) async {
    ...
    try {
      ...
      // invoke
      bool hasCCCD =
          await FlutterBlueUltra._invokePlatform(() => FlutterBlueUltraPlatform.instance.setNotifyValue(request));
      ...
      if (hasCCCD) {
        ...
        // failed?
        if (!response.success) {
          throw FlutterBlueUltraException(_nativeError, "setNotifyValue", response.errorCode, response.errorString);
        }
      }
    } finally {
      mtx.give();
  ...
  BmBluetoothService? get _bmsvc {
    if (FlutterBlueUltra._knownServices[remoteId] != null) {
      for (var s in FlutterBlueUltra._knownServices[remoteId]!.services) {
