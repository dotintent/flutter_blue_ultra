...
class BluetoothDevice {
  ...
  String get platformName => FlutterBlueUltra._platformNames[remoteId] ?? "";
  ...
  List<BluetoothService> get servicesList {
    BmDiscoverServicesResult? result = FlutterBlueUltra._knownServices[remoteId];
    if (result == null) {
      return [];
    } else {
  ...
  void cancelWhenDisconnected(StreamSubscription subscription, {bool next = false, bool delayed = false}) {
    if (isConnected == false && next == false) {
      ...
      FlutterBlueUltra._delayedSubscriptions[remoteId] ??= [];
      FlutterBlueUltra._delayedSubscriptions[remoteId]!.add(subscription);
    } else {
      FlutterBlueUltra._deviceSubscriptions[remoteId] ??= [];
      FlutterBlueUltra._deviceSubscriptions[remoteId]!.add(subscription);
    }
  }

  /// Returns true if autoConnect is currently enabled for this device
  bool get isAutoConnectEnabled {
    return FlutterBlueUltra._autoConnect.contains(remoteId);
  }

  /// Returns true if this device is currently connected to your app
  bool get isConnected {
    if (FlutterBlueUltra._connectionStates[remoteId] == null) {
      return false;
    } else {
      var state = FlutterBlueUltra._connectionStates[remoteId]!.connectionState;
  ...
  Future<void> connect({
    ...
    try {
      ...
      if (autoConnect) {
        FlutterBlueUltra._autoConnect.add(remoteId);
      }
      ...
      var responseStream =
          FlutterBlueUltraPlatform.instance.onConnectionStateChanged.where((p) => p.remoteId == remoteId);
      ...
      if (!kIsWeb && Platform.isAndroid) {
        FlutterBlueUltra._connectTimestamp[remoteId] = DateTime.now();
      }

      // invoke
      bool changed = await FlutterBlueUltra._invokePlatform(() => FlutterBlueUltraPlatform.instance.connect(request));
      ...
      if (changed && !autoConnect) {
        BmConnectionStateResponse response = await futureState
            ...
            .catchError((e) async {
          if (e is FlutterBlueUltraException && e.code == FbuErrorCode.timeout.index) {
            FlutterBlueUltraPlatform.log("[FBU] connection timeout");
            await FlutterBlueUltra._invokePlatform(() => FlutterBlueUltraPlatform.instance
                .disconnect(BmDisconnectRequest(remoteId: remoteId))); // cancel connection attempt
          }
          throw e;
        });
  ...
  Future<void> disconnect({
    ...
    try {
      ...
      FlutterBlueUltra._autoConnect.remove(remoteId);

      var responseStream = FlutterBlueUltraPlatform.instance.onConnectionStateChanged
      ...
      // invoke
      bool changed = await FlutterBlueUltra._invokePlatform(
          () => FlutterBlueUltraPlatform.instance.disconnect(BmDisconnectRequest(remoteId: remoteId)));
      ...
      if (!kIsWeb && Platform.isAndroid) {
        ...
        FlutterBlueUltra._connectTimestamp.remove(remoteId);
      }
    } finally {
      dtx.give();
      if (queue) {
  ...
  Future<List<BluetoothService>> discoverServices({bool subscribeToServicesChanged = true, int timeout = 15}) async {
    ...
    try {
      var responseStream = FlutterBlueUltraPlatform.instance.onDiscoveredServices.where((p) => p.remoteId == remoteId);

      // Start listening now, before invokeMethod, to ensure we don't miss the response
      Future<BmDiscoverServicesResult> futureResponse = responseStream.first;

      // invoke
      await FlutterBlueUltra._invokePlatform(
          () => FlutterBlueUltraPlatform.instance.discoverServices(BmDiscoverServicesRequest(remoteId: remoteId)));
      ...
      // failed?
      if (!response.success) {
        throw FlutterBlueUltraException(_nativeError, "discoverServices", response.errorCode, response.errorString);
      }
  ...
  DisconnectReason? get disconnectReason {
    ...
    int? code = FlutterBlueUltra._connectionStates[remoteId]!.disconnectReasonCode;
    String? description = FlutterBlueUltra._connectionStates[remoteId]!.disconnectReasonString;
  ...
  Stream<BluetoothConnectionState> get connectionState {
    ...
    if (FlutterBlueUltra._connectionStates[remoteId] != null) {
      initialValue = _bmToConnectionState(FlutterBlueUltra._connectionStates[remoteId]!.connectionState);
    }
    return FlutterBlueUltraPlatform.instance.onConnectionStateChanged
        .where((p) => p.remoteId == remoteId)
  ...
  int get mtuNow {
    ...
    return FlutterBlueUltra._mtuValues[remoteId]?.mtu ?? 23;
  }
  ...
  Stream<int> get mtu {
    // get initial value from our cache
    int initialValue = FlutterBlueUltra._mtuValues[remoteId]?.mtu ?? 23;
    return FlutterBlueUltraPlatform.instance.onMtuChanged
        .where((p) => p.remoteId == remoteId)
        .map((p) => p.mtu)
        .newStreamWithInitialValue(initialValue);
  }
  ...
  Stream<void> get onServicesReset {
    return FlutterBlueUltraPlatform.instance.onServicesReset
        .where((p) => p.remoteId == remoteId)
  ...
  Future<int> readRssi({int timeout = 15}) async {
    ...
    try {
      var responseStream = FlutterBlueUltraPlatform.instance.onReadRssi.where((p) => (p.remoteId == remoteId));
      ...
      // invoke
      await FlutterBlueUltra._invokePlatform(
          () => FlutterBlueUltraPlatform.instance.readRssi(BmReadRssiRequest(remoteId: remoteId)));
      ...
      // failed?
      if (!response.success) {
        throw FlutterBlueUltraException(_nativeError, "readRssi", response.errorCode, response.errorString);
      }
      rssi = response.rssi;
    } finally {
      mtx.give();
  ...
  Future<int> requestMtu(int desiredMtu, {double predelay = 0.35, int timeout = 15}) async {
    ...
    try {
      ...
      var responseStream =
          FlutterBlueUltraPlatform.instance.onMtuChanged.where((p) => p.remoteId == remoteId).map((p) => p.mtu);

      // Start listening now, before invokeMethod, to ensure we don't miss the response
      Future<int> futureResponse = responseStream.first;

      // invoke
      await FlutterBlueUltra._invokePlatform(() => FlutterBlueUltraPlatform.instance.requestMtu(request));
  ...
  Future<void> requestConnectionPriority({required ConnectionPriority connectionPriorityRequest}) async {
    ...
    // invoke
    await FlutterBlueUltra._invokePlatform(() => FlutterBlueUltraPlatform.instance.requestConnectionPriority(request));
  }
  ...
  Future<void> setPreferredPhy({
    ...
    // invoke
    await FlutterBlueUltra._invokePlatform(() => FlutterBlueUltraPlatform.instance.setPreferredPhy(request));
  }
  ...
  Future<void> createBond({int timeout = 90, Uint8List? pin}) async {
    ...
    try {
      var responseStream = FlutterBlueUltraPlatform.instance.onBondStateChanged
          .where((p) => p.remoteId == remoteId)
      ...
      // invoke
      bool changed = await FlutterBlueUltra._invokePlatform(
          () => FlutterBlueUltraPlatform.instance.createBond(BmCreateBondRequest(remoteId: remoteId, pin: pin)));
  ...
  Future<void> removeBond({int timeout = 30}) async {
    ...
    try {
      ...
      // invoke
      bool changed = await FlutterBlueUltra._invokePlatform(
          () => FlutterBlueUltraPlatform.instance.removeBond(BmRemoveBondRequest(remoteId: remoteId)));
  ...
  Future<void> clearGattCache() async {
    ...
    // invoke
    await FlutterBlueUltra._invokePlatform(
        () => FlutterBlueUltraPlatform.instance.clearGattCache(BmClearGattCacheRequest(remoteId: remoteId)));
  }
  ...
  Stream<BluetoothBondState> get bondState async* {
    ...
    if (FlutterBlueUltra._bondStates[remoteId] == null) {
      var val = await FlutterBlueUltra._invokePlatform(
          () => FlutterBlueUltraPlatform.instance.getBondState(BmBondStateRequest(remoteId: remoteId)));
      // update _bondStates if it is still null after the await
      if (FlutterBlueUltra._bondStates[remoteId] == null) {
        FlutterBlueUltra._bondStates[remoteId] = val;
      }
    }

    yield* FlutterBlueUltraPlatform.instance.onBondStateChanged
        .where((p) => p.remoteId == remoteId)
        .map((p) => _bmToBondState(p.bondState))
        .newStreamWithInitialValue(_bmToBondState(FlutterBlueUltra._bondStates[remoteId]!.bondState));
  }

  /// Get the previous bondState of the device (Android Only)
  BluetoothBondState? get prevBondState {
    var b = FlutterBlueUltra._bondStates[remoteId]?.prevState;
  ...
  Future<void> _ensureAndroidDisconnectionDelay(int androidDelay) async {
    if (!kIsWeb && Platform.isAndroid) {
      if (FlutterBlueUltra._connectTimestamp.containsKey(remoteId)) {
        Duration minGap = Duration(milliseconds: androidDelay);
        Duration elapsed = DateTime.now().difference(FlutterBlueUltra._connectTimestamp[remoteId]!);
        if (elapsed.compareTo(minGap) < 0) {
          Duration timeLeft = minGap - elapsed;
          FlutterBlueUltraPlatform.log(
              "[FBU] disconnect: enforcing ${minGap.inMilliseconds}ms disconnect gap, delaying "
  ...
  String toString() {
    return 'BluetoothDevice{'
        ...
        'services: ${FlutterBlueUltra._knownServices[remoteId]}'
        '}';
  }
