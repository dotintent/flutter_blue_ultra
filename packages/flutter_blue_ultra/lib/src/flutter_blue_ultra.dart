...
class FlutterBlueUltra {
  ...
  static Future<bool> get isSupported async =>
      await _invokePlatform(() => FlutterBlueUltraPlatform.instance.isSupported(BmIsSupportedRequest()));
  ...
  static Future<String> get adapterName async =>
      await _invokePlatform(() => FlutterBlueUltraPlatform.instance.getAdapterName(BmBluetoothAdapterNameRequest()))
  ...
  static Future<void> setOptions({
    ...
    await _invokePlatform(() => FlutterBlueUltraPlatform.instance
  ...
  static Future<void> turnOn({int timeout = 60}) async {
    var responseStream = FlutterBlueUltraPlatform.instance.onTurnOnResponse;

    // Start listening now, before invokeMethod, to ensure we don't miss the response
    Future<BmTurnOnResponse> futureResponse = responseStream.first;

    // invoke
    bool changed = await _invokePlatform(() => FlutterBlueUltraPlatform.instance.turnOn(BmTurnOnRequest()));
  ...
  static Stream<BluetoothAdapterState> get adapterState async* {
    ...
    if (_adapterStateNow == null) {
      var result = await _invokePlatform(
          () => FlutterBlueUltraPlatform.instance.getAdapterState(BmBluetoothAdapterStateRequest()));
      // update _adapterStateNow if it is still null after the await
      _adapterStateNow ??= result.adapterState;
    }

    yield* FlutterBlueUltraPlatform.instance.onAdapterStateChanged
  ...
  static Future<List<BluetoothDevice>> systemDevices(List<Guid> withServices) async {
    var r = await _invokePlatform(
        () => FlutterBlueUltraPlatform.instance.getSystemDevices(BmSystemDevicesRequest(withServices: withServices)));
  ...
  static Future<List<BluetoothDevice>> get bondedDevices async {
    var r = await _invokePlatform(() => FlutterBlueUltraPlatform.instance.getBondedDevices(BmBondedDevicesRequest()));
    for (BmBluetoothDevice device in r.devices) {
      if (device.platformName != null) {
        _platformNames[device.remoteId] = device.platformName!;
      }
  ...
  static Future<void> startScan({
    ...
    try {
      ...
      Stream<BmScanResponse> responseStream = FlutterBlueUltraPlatform.instance.onScanResponse;
      ...
      await _invokePlatform(() => FlutterBlueUltraPlatform.instance.startScan(settings)).onError((e, s) {
      ...
      _scanSubscription = outputStream.listen((BmScanResponse? response) {
        if (response == null) {
          ...
          if (response.success == false) {
            var e = FlutterBlueUltraException(_nativeError, "scan", response.errorCode, response.errorString);
  ...
  static Future<void> stopScan() async {
    ...
    try {
      if (isScanningNow) {
        ...
        FlutterBlueUltraPlatform.log("[FBU] stopScan: already stopped");
      }
    } finally {
      mtx.give();
    }
  ...
  static Future<void> _stopScan({bool invokePlatform = true}) async {
    ...
    if (invokePlatform) {
      await _invokePlatform(() => FlutterBlueUltraPlatform.instance.stopScan(BmStopScanRequest()));
    }
  }
  ...
  static void cancelWhenScanComplete(StreamSubscription subscription) {
    FlutterBlueUltra._scanSubscriptions.add(subscription);
  }
  ...
  static Future<void> setLogLevel(LogLevel level, {color = true}) async {
    ...
    await _invokePlatform(
        () => FlutterBlueUltraPlatform.instance.setLogLevel(BmSetLogLevelRequest(logLevel: level, logColor: color)));
  }
  ...
  static Future<void> _initFlutterBlueUltra() async {
    if (_initialized) {
      return;
    }
    ...
    // android only
    if (!kIsWeb && Platform.isAndroid) {
      FlutterBlueUltraPlatform.instance.onDetachedFromEngine.listen((r) {
        _stopScan(invokePlatform: false);
      });
    }

    // keep track of adapter states
    FlutterBlueUltraPlatform.instance.onAdapterStateChanged.listen((r) {
        ...
        for (DeviceIdentifier d in _autoConnect) {
          BluetoothDevice(remoteId: d).connect(autoConnect: true, mtu: null).onError((e, s) {
            if (logLevel != LogLevel.none) {
              FlutterBlueUltraPlatform.log("[FBU] [AutoConnect] connection failed: $e");
            }
          });
        }
      }
    ...
    FlutterBlueUltraPlatform.instance.onConnectionStateChanged.listen((r) {
    ...
    FlutterBlueUltraPlatform.instance.onNameChanged.listen((r) {
      _platformNames[r.remoteId] = r.name;
    });

    // keep track of services resets
    FlutterBlueUltraPlatform.instance.onServicesReset.listen((r) {
      _knownServices.remove(r.remoteId);
    });

    // keep track of bond state
    FlutterBlueUltraPlatform.instance.onBondStateChanged.listen((r) {
      _bondStates[r.remoteId] = r;
    });

    // keep track of services
    FlutterBlueUltraPlatform.instance.onDiscoveredServices.listen((r) {
      if (r.success == true) {
    ...
    FlutterBlueUltraPlatform.instance.onMtuChanged.listen((r) {
      if (r.success == true) {
    ...
    _mergeStreams([
      FlutterBlueUltraPlatform.instance.onCharacteristicReceived,
      FlutterBlueUltraPlatform.instance.onCharacteristicWritten
    ]).listen((r) {
      if (r.success == true) {
  ...
  static Future<void> turnOff({int timeout = 10}) async {
    var responseStream =
        FlutterBlueUltraPlatform.instance.onAdapterStateChanged.where((p) => p.adapterState == BmAdapterStateEnum.off);

    // Start listening now, before invokeMethod, to ensure we don't miss the response
    Future<BmBluetoothAdapterState> futureResponse = responseStream.first;

    // invoke
    bool changed = await _invokePlatform(() => FlutterBlueUltraPlatform.instance.turnOff(BmTurnOffRequest()));
...
class FlutterBlueUltraException implements Exception {
  ...
  FlutterBlueUltraException(this.platform, this.function, this.code, this.description);

  @override
  String toString() {
    String sPlatform = platform.toString().split('.').last;
    return 'FlutterBlueUltraException | $function | $sPlatform-code: $code | $description';
  }
