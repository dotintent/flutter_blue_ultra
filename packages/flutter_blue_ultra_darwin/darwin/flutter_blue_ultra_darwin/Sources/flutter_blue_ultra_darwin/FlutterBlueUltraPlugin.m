...
@interface CBUUID (CBUUIDAdditionsFlutterBlueUltra)
- (NSString *)uuidStr;
@end

@implementation CBUUID (CBUUIDAdditionsFlutterBlueUltra)
- (NSString *)uuidStr
{
...
@interface FlutterBlueUltraPlugin ()
...
- (NSNumber *)getInstanceId:(CBCharacteristic *)characteristic
{
    ...
    CBUUID *needle = characteristic.UUID;
    NSInteger idx = 0;

    for (CBCharacteristic *c in svc.characteristics) {
        if ([c.UUID isEqual:needle]) {
            if (c == characteristic) return @(idx);
            idx++;
        }
    }
    return @(0);
