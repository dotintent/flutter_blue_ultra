...
extension on BlueZGattCharacteristic {
  int instanceId(BlueZGattService service) {
    ...
    for (final c in service.characteristics) {
      if (Guid.fromBytes(c.uuid.value) == target) {
        if (identical(c, this)) return idx;
        idx++;
      }
    }
    return 0;
...
final class FlutterBlueUltraLinux extends FlutterBlueUltraPlatform {
  ...
  static void registerWith() {
    FlutterBlueUltraPlatform.instance = FlutterBlueUltraLinux();
  }

  Future<void> _initFlutterBlueUltra() async {
    if (_initialized) {
      return;
    }
      ...
      (devices) {
        if (_logLevel == LogLevel.verbose) {
          FlutterBlueUltraPlatform.log(
            '[FBU-Linux] devices changed ${devices.map((device) => device.remoteId).toList()}',
          );
        }
            ...
            (device) {
              return device.propertiesChanged.switchMap(
                (properties) {
                  if (_logLevel == LogLevel.verbose) {
                    FlutterBlueUltraPlatform.log(
                      '[FBU-Linux] device ${device.remoteId} properties changed $properties',
                    );
                  }
                      ...
                      streams.add(
                        characteristic.propertiesChanged.map(
                          (properties) {
                            if (_logLevel == LogLevel.verbose) {
                              FlutterBlueUltraPlatform.log(
                                '[FBU-Linux] device ${device.remoteId} service ${service.uuid} characteristic ${characteristic.uuid} properties changed $properties',
                              );
                            }
                          },
                        ),
